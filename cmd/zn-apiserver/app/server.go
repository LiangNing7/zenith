...
import (
	...
	"github.com/LiangNing7/zenith/cmd/zn-apiserver/app/options"
	"github.com/LiangNing7/zenith/internal/pkg/log"
)

var configFile string // 配置文件路径.

// NewZenithCommand 创建一个 *cobra.Command 对象，用于启动应用程序.
func NewZenithCommand() *cobra.Command {
	// 创建默认的应用命令行选项.
...
• High-quality code.`,
		...
		// 设置命令运行时的参数检查，不需要指定命令行参数。例如：./zenith param1 param2
		Args: cobra.NoArgs,
	}
	// 初始化配置函数，在每个命令运行时调用.
	cobra.OnInitialize(core.OnInitialize(&configFile, "ZENITH", searchDirs(), defaultConfigName))

	// cobra 支持持久性标志(PersistentFlag)，该标志可用于它所分配的命令以及该命令下的每个子命令.
	// 推荐使用配置文件来配置应用，便于管理配置项.
	cmd.PersistentFlags().StringVarP(&configFile, "config", "c", filePath(), "Path to the zenith configuration file.")
...
// run 是运行逻辑，负者初始化日志，解析配置、校验选项并启动服务器.
