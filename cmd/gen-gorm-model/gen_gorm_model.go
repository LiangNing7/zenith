...
var generateConfigs = map[string]GenerateConfig{
	"mb": {ModelPackagePath: "../../internal/apiserver/model", GenerateFunc: GenerateZenithModels},
}

// 命令行参数.
var (
	addr       = pflag.StringP("addr", "a", "127.0.0.1:3306", "MySQL host address.")
	username   = pflag.StringP("username", "u", "zenith", "Username to connect to the database.")
	password   = pflag.StringP("password", "p", "zenith1234", "Password to use when connecting to the database.")
	database   = pflag.StringP("db", "d", "zenith", "Database name to connect to.")
...
// GenerateZenithModels 为 zenith 组件生成模型.
func GenerateZenithModels(g *gen.Generator) {
	g.GenerateModelAs(
		"user",
		"UserM",
