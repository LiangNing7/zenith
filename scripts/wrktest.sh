...
2. 执行命令： ./wrktest.sh diff zn-apiserver.dat http.dat
...
OPTIONS:
  -h                     Usage information
  -n                     Performance test task name, default: zn-apiserver
...
function mb::wrk::convert_plot_data()
{
...
function mb::wrk::prepare()
{
  rm -f ${wrkdir}/${datfile}
}

# Plot according to gunplot data file
function mb::wrk::plot() {
  gnuplot <<  EOF
...
case $1 in
  "diff")
    ...
    mb::wrk::plot_diff `basename $2` `basename $3`
    exit 0
    ;;
  *)
    if [ "$#" -lt 1 ];then
    ...
    mb::wrk::setup
    mb::wrk::start_performance_test "${url}"
    ;;
esac
