...
import (
	...
	handler "github.com/LiangNing7/zenith/internal/apiserver/handler/grpc"
	mw "github.com/LiangNing7/zenith/internal/pkg/middleware/grpc"
	"github.com/LiangNing7/zenith/internal/pkg/server"
	apiv1 "github.com/LiangNing7/zenith/pkg/api/apiserver/v1"
)

// grpcServer 定义了一个 gRPC 服务器.
...
func (c *ServerConfig) NewGRPCServerOr() (server.Server, error) {
	...
	grpcsrv, err := server.NewGRPCServer(
		...
		func(s grpc.ServiceRegistrar) {
			apiv1.RegisterZenithServer(s, handler.NewHandler(c.biz))
		},
	)
	if err != nil {
		return nil, err
	...
	httpsrv, err := server.NewGRPCGatewayServer(
		...
		func(mux *runtime.ServeMux, conn *grpc.ClientConn) error {
			return apiv1.RegisterZenithHandler(context.Background(), mux, conn)
		},
	)
	if err != nil {
		return nil, err
