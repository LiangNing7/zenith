...
zenith（微博客） 是一个 Go 语言入门项目，用来完成用户注册、博客创建等业务功能。zenith 入门但不简单：
...
```bash
$ git clone https://github.com/LiangNing7/zenith.git # 克隆项目
$ cd zenith
$ go work init . # 初始化 Go 工作区
$ go work use .
$ make build BINS=zn-apiserver # 编译源码
$ _output/platforms/linux/amd64/zn-apiserver
...
zn-apiserver 支持内存数据库和 MySQL 数据库，默认使用内存数据库启动。如果你想切换为 MySQL 数据库请按以下方式准备一个 MySQL 数据库：
- 您可以根据 [安装和配置 MariaDB 数据库](./docs/devel/zh-CN/mysql.md) 文档中的指引，安装并初始化一个 `zenith` 数据库；
- 如果您有自己的数据库，请使用 `configs/zenith.sql` 中的 SQL 初始化 zenith 依赖的数据库及表，并更新 `configs/zn-apiserver.yaml` 文件中 `mysql` 配置项中的 `host`、`username`、`password`、`database`。

并修改 configs/zn-apiserver.yaml 文件中的 `enable-memory-store` 为 `false`。
